...
def process(request):
    ...
    try:
        ...
        raise
    else:return HttpResponseRedirect('./')from django.http import HttpResponse, HttpResponseRedirect
from django.shortcuts import get_object_or_404, render
from django.utils import timezone
...
def process(request):
    ...
    try:
        ...
        for word in KeyWord.objects.all():
            magic_list[word.word] = int(word.weight)
        score = 0
        ...
        parsed_file = json.loads(json_data)
        full_resume = ("\n".join(resDict.values())).lower()
        if len(parsed_file["items"]) < sites: sites = len(parsed_file["items"]) 
        for i in range(sites):
            if "linkedin" not in parsed_file["items"][i]["link"]:
                web_file = urllib2.urlopen(parsed_file["items"][i]["link"])
                print("\"" + parsed_file["items"][i]["link"] + "\" crawled...")
